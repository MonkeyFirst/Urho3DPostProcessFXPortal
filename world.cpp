...
void World::InitScene(Context* context_, String sceneFileNam, EWORLDS worldType_) 
{
	...
	File sceneFile(context_, context_->GetSubsystem<FileSystem>()->GetProgramDir() + "Data/Scenes/" + sceneFileNam, FILE_READ);
...
void World::MoveCamera(float timeStep) 
{
	...
	if (dir.Length() > 0)
	{
		...
		if (curVel.Length() < 5.0f)
		{
			...
			player.body->SetLinearVelocity(multipler * 500.0f * (camRotation * dir) * timeStep);
		}
	}
}